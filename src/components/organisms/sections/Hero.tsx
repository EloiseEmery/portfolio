...
function Hero({ language }: { language: Language }) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (decorativeEl) {
        decorativeEl.style.transform = `translateY(${scrollPosition * 0.2}px)`;
      }
      if (paralaxeEl) {
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute -top-[50px] -right-[50px] md:right-8 md:top-0 z-0" id="paralaxeEl">
        <img src={paralaxeEl} alt="Paralaxe Element" className="w-full max-h-[350px] opacity-50" />
      </div>
      {/* Hero Texts */}
